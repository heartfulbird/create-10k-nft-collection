...
const ipfsMetasFile = `${basePath}/build/real/ipfsMetas/_ipfsMetas.json`;
...
async function main() {
  ...
  if (!fs.existsSync(path.join(`${basePath}/build/real`, "/minted"))) {
    fs.mkdirSync(path.join(`${basePath}/build/real`, "minted"));
  }
  ...
  for (const meta of ipfsMetas) {
    ...
    const mintFile = `${basePath}/build/real/minted/${edition}.json`;
    ...
    try {
      ...
      try {
        ...
        fs.writeFileSync(
          `${basePath}/build/real/minted/${edition}.json`,
  ...
  console.log("Minting complete. To check for errors run command: npm run check_txns --dir=minted (set proper dir for real)");
}
