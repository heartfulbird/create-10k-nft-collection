...
const ipfsMetasFile = `${basePath}/build/hidden/ipfsMetas/_ipfsMetas.json`;

// TODO: CONTRACT IN STANDARD FOLDER (CAUSE SAME FOR hidden and real)
...
async function main() {
  ...
  if (!fs.existsSync(path.join(`${basePath}/build/hidden`, "/minted"))) {
    fs.mkdirSync(path.join(`${basePath}/build/hidden`, "minted"));
  }
  ...
  for (const meta of ipfsMetas) {
    ...
    const mintFile = `${basePath}/build/hidden/minted/${edition}.json`;
    ...
    try {
      ...
      if (mintedFile.length > 0) {
        ...
        if (mintedMeta.mintData.response !== "OK") {
          ...
          console.log(`${meta.name} already minted, (edition: ${edition})`);
        } else {
      ...
      try {
        ...
        fs.writeFileSync(
          `${basePath}/build/hidden/minted/${edition}.json`,
        ...
        if (mintData.response !== "OK") {
          ...
          console.log(`Mint transaction created for: ${meta.name}!, (edition: ${edition})`);
        }
      } catch (err) {
        console.log(`Catch: Minting ${meta.name} failed with ${err}!, (edition: ${edition})`);
      }
    }
  }

  console.log("Minting complete. To check for errors run command: npm run check_txns --dir=minted (set proper dir for hidden)");
}
